...
def main() -> None:
    ...
    summary, manual, manual_preview, manual_panel, manual_gallery = client.predict(
        "detect_open_vocab", handle_file(str(source)), "yellow safety helmet", 128,
        0, 0, 320, 320, api_name="/execute_tool"
    )
    ...
    summary2, mask_result, mask_preview, mask_panel, mask_gallery = client.predict(
        "segment_objects", handle_file(str(source)), "unused", 128,
        box["x1"], box["y1"], box["x2"], box["y2"], api_name="/execute_tool"
    )
